...
export const calculateMatchingScore = (job, jobSeekers) => {
  let score = 0;

  // Increase the score for each matching skill
  job.Skills.forEach((skill) => {
    if (jobSeekers.skills && jobSeekers.skills.includes(skill)) {
      score++;
    }
  });

  // Increase the score if the location matches
  if (jobSeekers.address && jobSeekers.address === job.Location) {
    score++;
  }

  // Increase the score if the contract type matches
  if (jobSeekers.contractPreference && jobSeekers.contractPreference === job.Contract) {
    score++;
  }

  // Increase the score if the worksite type matches
  if (jobSeekers.worksitePreference && jobSeekers.worksitePreference === job.WorkSite) {
    score++;
  }
...
function ViewApplications() {
  ...
  const [jobSeekers, setJobSeekers] = useState([]); // Add this line
  ...
  useEffect(() => {
    const getJobSeekers = async () => {
      ...
      setJobSeekers(
        jobSeekersSnapshot.docs.map((doc) => ({
          ...doc.data(),
          id: doc.id,
        })),
      );
  ...
  const handleViewApplications = (jobId) => {
    ...
    navigate(`/view2/${jobId}`, { state: { matchedJobSeekers } });
    };
  ...
  .filter((job) => {
   ...
   return (
    ...
    <div className="d-flex flex-wrap justify-content-between mb-2 w-50">
       <div className="filter-container">
            <div className="filters mb-2 w-100">
         <input
           type="text"
          ...
          <Card.Body>
            <div className="d-flex justify-content-between">
              <div>
                ...
                <Card.Title>
                  {job.JobTitle} -{' '}
                  <span className="text-mute font-weight-light">{job.Company} </span>
                </Card.Title>
                <Card.Subtitle>{new Date(job.date).toLocaleDateString()}</Card.Subtitle>
