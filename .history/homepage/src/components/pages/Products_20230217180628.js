 ...
 const filteredjobs = jobs.filter(filterFunc) ;
  ...
  <div className='App'>
    ...
    <div className={`flex bg-white shadow-md my-16 mx-10 p-4 rounded`}>
      ...
      {
          filters.length > 0 && filters.map((filter) => <span onClick = {()=> handleFilterClick(filter)}className="text-blue-100 bg-blue-400 font-bold m-3 p-2 rounded cursor-pointer">{filter}</span>)
