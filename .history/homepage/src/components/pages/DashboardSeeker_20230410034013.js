...
function DashboardSeeker() {
    ...
    const [skillDropdowns, setSkillDropdowns] = useState([{ id: 0, selectedSkills: [] }]);
...
const handleCareerInfoSubmit = async (event) => {
  ...
  // Extract all selected skills from every dropdown menu
  const allSelectedSkills = skillDropdowns.flatMap((dropdown) => dropdown.selectedSkills);
  ...
  await updateDoc(jobSeekerRef, {
    ...
    contractPreference,
    worksitePreference,
    skills: allSelectedSkills, // Store all the selected skills from every dropdown menu
  });
...
const addSkillDropdown = () => {
  setSkillDropdowns([...skillDropdowns, { id: skillDropdowns.length, selectedSkills: [] }]);
};

const handleSkillChange = (dropdownId, e) => {
  const selectedOptions = Array.from(e.target.selectedOptions);
  const newSelectedSkills = selectedOptions.map((option) => option.value);

  setSkillDropdowns(
    skillDropdowns.map((dropdown) =>
      dropdown.id === dropdownId ? { ...dropdown, selectedSkills: newSelectedSkills } : dropdown
    )
  );
};
...
const removeSkill = (skillToRemove) => {
  setSkillDropdowns(
    skillDropdowns.map((dropdown) => ({
      ...dropdown,
      selectedSkills: dropdown.selectedSkills.filter((skill) => skill !== skillToRemove),
    }))
  );
};
        ...
        >
          <Card.Body>
            ...
            <Form onSubmit={handleCareerInfoSubmit}>
              ...
              <Form>
                {skillDropdowns.map((dropdown, index) => (
                  <Form.Group
                    key={dropdown.id}
                    controlId={`formSkill${dropdown.id}`}
                    className="mt-3"
                  >
                    ...
                    <Form.Control
                      as="select"
                      multiple
                      value={dropdown.selectedSkills}
                      onChange={(e) => handleSkillChange(dropdown.id, e)}
                    >
                      <option disabled value="">
                ...
                <Button
                  ...
                  type="button"
                  onClick={addSkillDropdown}
                >
                  + Add another skill dropdown
                </Button>
                <div>
                  {skillDropdowns
                    .flatMap((dropdown) => dropdown.selectedSkills)
                    .map((skill, index) => (
                      <Badge
                        key={index}
                        pill
