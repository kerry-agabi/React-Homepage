...
function JobCard() {
  ...
  useEffect(() => {
    const parseSkillsCSV = async () => {
      const response = await fetch(skillsCSV);
      const csvText = await response.text();
      Papa.parse(csvText, {
        header: true,
        complete: (results) => {
          setSkillsOptions(results.data.map((skill) => ({ label: skill.name, value: skill.name })));
        },
      });
    };
  ...
  const [skillsOptions, setSkillsOptions] = useState([{ selected: "" }])
         ...
         <div className="filter-container">
           <div className="filters mb-4 w-100">
             ...
             <Select
               ...
               value={skillsFilter}
               onChange={(option) =>
                 setSkillsFilter(option ? option.value : "")
               }
