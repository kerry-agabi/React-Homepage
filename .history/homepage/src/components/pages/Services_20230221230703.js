...

{
  ...
  return (
    <div className={`flex bg-white shadow-lg m-4 p-3 rounded ${featured && 'border-l-8 border-black border-solid'}`}>
      <div>
      ...
      <div className="flex flex-col justify-between ml-4 ">
        <h3 className="font-bold text-base text-blue-500">
          {company}
          {isNew && <span className='text-blue-100 bg-blue-500 font-bold m-2 py-1 px-2 rounded-full'>New</span>}
        ...
        <p className="text-gray-700">
          ...
          <Button className='text-blue-100 bg-blue-400 m-0.5 p-2 rounded cursor-pointer text-7xs'> APPLY NOW </Button>
        </p>
      </div>
      ...
      <div className="bg-flex item-center ml-auto">
        {tags
          ? tags.map((tag) => (
              <span onClick = {()=> handleTagClick(tag)}className="text-blue-100 bg-blue-400 font-bold m-3 p-2 cursor-pointer rounded">
                {tag}
              </span>
            ))
          : ""}
