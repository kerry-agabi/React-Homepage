...
function JobCard() {
  ...
  const handleFilterLocation = location => {
    const filteredJobs = jobs.filter(job => job.Location === location);
    setjobs(filteredJobs);
  };

  const handleFilterContract = contract => {
    const filteredJobs = jobs.filter(job => job.Contract === contract);
    setjobs(filteredJobs);
  };
...
<Button
          variant="secondary"
          className="mr-2 mb-2"
          onClick={() => handleFilterLocation('New York')}
        >
          New York
        </Button>
        ...
        <Button
          variant="secondary"
          className="mr-2 mb-2"
          onClick={() => handleFilterLocation('Full Time')}>
          R
        </Button>
