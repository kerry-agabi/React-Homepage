...
import { collection, getDocs, setDoc, addDoc, updateDoc, doc, deleteDoc } from 'firebase/firestore';
...
function DashboardSeeker() {
  ...
  const handleEducationSubmit = async (event, index) => {
    ...
    await updateDoc(jobSeekerRef, {
      [`educations.${index}`]: educations[index],
    });
    ...
    const handleSubmit = async (event) => {
      ...
      const jobSeekerRef = doc(db, "jobSeekers", currentUser.uid);
      const jobSeekerSnapshot = await jobSeekerRef.get();
      if (jobSeekerSnapshot.exists()) {
        const jobSeekerData = jobSeekerSnapshot.data();
        if (jobSeekerData.cvUrl) {
          const oldCvRef = ref(storage, jobSeekerData.cvUrl);
