...
export const calculateMatchingScore = (job, jobSeeker) => {
  ...
  if (jobSeeker.workExperiences && job && job.JobTitle) {
    const matchingExperience = jobSeeker.workExperiences.some(
      (experience) => experience.JobTitle === job.JobTitle
    );
    if (matchingExperience) {
      score++;
      matchedCredentials.push(`JobTitle: ${job.JobTitle}`);
    }
  }
