    ...
    <div className="App">
      ...
      {filters.length > 0 && (
        <div className={`flex bg-white shadow-md my-16 mx-10 p-4 rounded`}>
          {filters.map((filter) => (
            <span className='cursor-pointer  m-3 p-2 '
              onClick={() => handleFilterClick(filter)}>
                <span
              className="text-blue-100 bg-blue-400 font-bold rounded cursor-pointer">
            
              {filter}
              </span>
            <span className='text-blue-100 bg-blue-500 m-3 p-2 rounded cursor-pointer'> ✖ </span>
            </span>
          ))}
        </div>
      )}
