...
const handleProfilePhotoSubmit = async (e) => {
  ...
  try {
    // Get the current jobSeeker document from Firestore
    const jobSeekerDoc = await jobSeekersCollection.doc(currentUser.uid).get();
    const jobSeekerData = jobSeekerDoc.data();

    // Check if there's an existing profile photo and delete it from Firebase Storage
    if (jobSeekerData.profilePhoto) {
      const currentPhotoURL = jobSeekerData.profilePhoto;
      const currentPhotoRef = storage.refFromURL(currentPhotoURL);
      await currentPhotoRef.delete();
    }

    // Upload the new profile photo to Firebase Storage
    const storageRef = storage.ref();
    const profilePhotoRef = storageRef.child(`profilePhotos/${currentUser.uid}/${profilePhoto.name}`);
    const snapshot = await profilePhotoRef.put(profilePhoto);
    const photoURL = await snapshot.ref.getDownloadURL();

    // Update the jobSeeker document in Firestore with the new photo URL
    await jobSeekersCollection.doc(currentUser.uid).update({
