...
useEffect(() => {
  const getjobSeekers = async () => {
    ...
    const jobSeekersData = await Promise.all(
      jobSeekersSnapshot.docs.map(async (docSnapshot) => {
        const careerInfoSnapshot = await getDocs(doc(db, "jobSeekers", docSnapshot.id, "careerInfo"));
        const careerInfoData = careerInfoSnapshot.data();
        return {
          ...careerInfoData,
          id: docSnapshot.id,
        };
  ...
  const findMatchingjobSeekers = (job) => {
    const matchedjobSeekers = jobSeekers
      .map((jobSeekers) => {
        return {
          ...jobSeekers,
          matchingScore: calculateMatchingScore(job, jobSeekers),
        };
      })
      .filter((jobSeekers) => jobSeekers.matchingScore > 0)
